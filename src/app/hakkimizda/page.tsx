import type { Metadata } from "next";
import { SectionHeading } from "@/components/ui/section-heading";
import { CtaSection } from "@/components/home/cta-section";
import { ScrollReveal } from "@/components/animations/scroll-reveal";
import { StaggerReveal } from "@/components/animations/stagger-reveal";
import {
  Target,
  Eye,
  Shield,
  Factory,
  Flame,
  Users,
  TrendingUp,
} from "lucide-react";

export const metadata: Metadata = {
  title: "Hakkımızda",
  description:
    "Rotermi Enerji — 15 yıllık tecrübe ile endüstriyel yakma sistemlerinde güvenilir çözüm ortağınız. Misyonumuz, vizyonumuz ve değerlerimiz.",
};

const values = [
  {
    icon: Shield,
    title: "Güvenilirlik",
    description:
      "Her projede tutarlı kalite ve zamanında teslimat ile güven inşa ediyoruz.",
  },
  {
    icon: TrendingUp,
    title: "Yenilikçilik",
    description:
      "En son teknolojileri takip ederek, müşterilerimize yenilikçi çözümler sunuyoruz.",
  },
  {
    icon: Users,
    title: "Müşteri Odaklılık",
    description:
      "Her müşterinin ihtiyacına özel çözümler geliştiriyor, uzun vadeli iş birlikleri kuruyoruz.",
  },
  {
    icon: Flame,
    title: "Tutku",
    description:
      "Enerji sektörüne olan tutkumuz, her projede en iyisini sunmamızı sağlıyor.",
  },
];

const activityAreas = [
  "Termik Enerji Santralleri",
  "Alternatif Yakma Sistemleri",
  "Şeker Fabrikaları",
  "Biyokütle Santralleri",
  "Buhar Kazanları",
  "Çeşitli Projeler",
];

export default function HakkimizdaPage() {
  return (
    <>
      {/* Page Hero */}
      <section className="relative bg-navy-900 py-20 lg:py-24 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-navy-800/50 to-navy-950/80" />
        <div className="relative mx-auto max-w-7xl px-4 text-center">
          <span className="text-flame-400 text-sm font-semibold tracking-widest uppercase">
            Kurumsal
          </span>
          <h1 className="mt-4 text-4xl sm:text-5xl lg:text-6xl font-bold font-display text-white tracking-tight">
            Hakkımızda
          </h1>
          <p className="mt-4 text-lg text-warm-200/80 max-w-2xl mx-auto">
            Endüstriyel yakma sistemlerinde 15 yıllık tecrübe ile enerji
            sektörüne yön veriyoruz.
          </p>
        </div>
      </section>

      {/* About Content */}
      <section className="py-20 lg:py-28 bg-white">
        <div className="mx-auto max-w-7xl px-4">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <ScrollReveal>
              <div>
                <span className="text-flame-500 text-sm font-semibold tracking-widest uppercase">
                  Rotermi Enerji
                </span>
                <h2 className="mt-3 text-3xl sm:text-4xl font-bold text-navy-800 tracking-tight">
                  Enerji Sektöründe 15 Yıllık Güven
                </h2>
                <div className="mt-6 space-y-4 text-warm-600 leading-relaxed">
                  <p>
                    Rotermi Enerji olarak, endüstriyel yakma sistemleri alanında
                    uzman mühendis kadromuz ve geniş ürün portföyümüz ile
                    müşterilerimize anahtar teslim çözümler sunuyoruz.
                  </p>
                  <p>
                    Brülör, kazan, enerji verimliliği ve otomasyon alanlarındaki
                    derin bilgi birikimimiz sayesinde, her ölçekteki işletmenin
                    enerji ihtiyacına özel çözümler geliştiriyoruz.
                  </p>
                  <p>
                    Müşteri memnuniyetini ve sürdürülebilir enerji çözümlerini
                    temel prensip edinerek, Türkiye&apos;nin dört bir yanında ve
                    yurt dışında başarılı projelere imza attık.
                  </p>
                </div>
              </div>
            </ScrollReveal>

            {/* Visual side — stats grid */}
            <ScrollReveal direction="right" delay={0.15}>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { icon: Factory, value: "500+", label: "Proje" },
                  { icon: Users, value: "300+", label: "Müşteri" },
                  { icon: Shield, value: "15+", label: "Yıl Deneyim" },
                  { icon: Flame, value: "7", label: "Ürün Grubu" },
                ].map(({ icon: Icon, value, label }) => (
                  <div
                    key={label}
                    className="rounded-2xl bg-warm-50 border border-warm-200 p-6 text-center hover:border-navy-200 hover:shadow-md transition-all"
                  >
                    <Icon className="h-8 w-8 text-flame-500 mx-auto mb-3" />
                    <div className="text-2xl font-bold text-navy-800 font-display">
                      {value}
                    </div>
                    <div className="text-sm text-warm-500 mt-1">{label}</div>
                  </div>
                ))}
              </div>
            </ScrollReveal>
          </div>
        </div>
      </section>

      {/* Mission / Vision / Values */}
      <section id="misyon-vizyon" className="py-20 lg:py-28 bg-warm-50">
        <div className="mx-auto max-w-7xl px-4">
          <StaggerReveal className="grid sm:grid-cols-2 gap-8 mb-16" staggerDelay={0.15}>
            <div className="rounded-2xl bg-white border border-warm-200 p-8">
              <div className="w-12 h-12 rounded-xl bg-navy-50 flex items-center justify-center mb-5">
                <Target className="h-6 w-6 text-navy-600" />
              </div>
              <h3 className="text-xl font-semibold text-navy-800">Misyonumuz</h3>
              <p className="mt-3 text-warm-600 leading-relaxed">
                Endüstriyel yakma sistemleri ve enerji çözümlerinde,
                müşterilerimize en yüksek kalitede ürün ve hizmet sunarak,
                güvenilir çözüm ortağı olmak.
              </p>
            </div>
            <div className="rounded-2xl bg-white border border-warm-200 p-8">
              <div className="w-12 h-12 rounded-xl bg-flame-50 flex items-center justify-center mb-5">
                <Eye className="h-6 w-6 text-flame-600" />
              </div>
              <h3 className="text-xl font-semibold text-navy-800">Vizyonumuz</h3>
              <p className="mt-3 text-warm-600 leading-relaxed">
                Enerji sektöründe yenilikçi ve sürdürülebilir çözümlerle
                Türkiye&apos;nin ve bölgenin lider endüstriyel yakma sistemleri
                firması olmak.
              </p>
            </div>
          </StaggerReveal>

          {/* Values grid */}
          <ScrollReveal>
            <SectionHeading
              label="Değerlerimiz"
              title="Bizi Biz Yapan Değerler"
            />
          </ScrollReveal>
          <StaggerReveal className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6" staggerDelay={0.08}>
            {values.map(({ icon: Icon, title, description }) => (
              <div
                key={title}
                className="rounded-xl bg-white border border-warm-200 p-6 text-center hover:shadow-md transition-shadow"
              >
                <div className="w-12 h-12 rounded-xl bg-navy-50 flex items-center justify-center mx-auto mb-4">
                  <Icon className="h-6 w-6 text-navy-600" />
                </div>
                <h3 className="font-semibold text-navy-800">{title}</h3>
                <p className="mt-2 text-sm text-warm-600">{description}</p>
              </div>
            ))}
          </StaggerReveal>
        </div>
      </section>

      {/* Activity Areas */}
      <section id="faaliyet-alanlari" className="py-20 lg:py-28 bg-white">
        <div className="mx-auto max-w-7xl px-4">
          <ScrollReveal>
            <SectionHeading
              label="Faaliyet Alanları"
              title="Hizmet Verdiğimiz Sektörler"
              description="Endüstriyel yakma sistemleri uzmanlığımızla farklı sektörlere özel çözümler sunuyoruz."
            />
          </ScrollReveal>
          <StaggerReveal className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-4" staggerDelay={0.06}>
            {activityAreas.map((area) => (
              <div
                key={area}
                className="flex items-center gap-3 rounded-xl bg-warm-50 border border-warm-200 px-5 py-4 hover:border-navy-200 hover:bg-navy-50/30 transition-all"
              >
                <div className="w-2 h-2 rounded-full bg-flame-500 shrink-0" />
                <span className="text-sm font-medium text-navy-800">
                  {area}
                </span>
              </div>
            ))}
          </StaggerReveal>
        </div>
      </section>

      {/* Quality section */}
      <section id="kalite" className="py-20 lg:py-28 bg-warm-50">
        <div className="mx-auto max-w-7xl px-4">
          <ScrollReveal>
            <div className="rounded-2xl bg-gradient-to-r from-navy-800 to-navy-900 p-8 sm:p-12 text-white">
              <div className="flex items-center gap-4 mb-6">
                <Shield className="h-8 w-8 text-flame-400" />
                <h2 className="text-2xl sm:text-3xl font-bold">
                  Kalite Politikamız
                </h2>
              </div>
              <div className="grid sm:grid-cols-2 gap-6 text-white/80 leading-relaxed">
                <p>
                  Rotermi Enerji olarak, tüm süreçlerimizde ISO 9001 Kalite
                  Yönetim Sistemi standartlarını esas alıyor; müşteri
                  memnuniyetini, sürekli iyileştirmeyi ve yasal mevzuata tam
                  uyumu temel prensip olarak benimsiyoruz.
                </p>
                <p>
                  Ürün ve hizmetlerimizin her aşamasında kalite kontrol
                  süreçlerimizi titizlikle uyguluyor, tedarikçiden son
                  kullanıcıya kadar uzanan değer zincirinde en yüksek
                  standartları hedefliyoruz.
                </p>
              </div>
            </div>
          </ScrollReveal>
        </div>
      </section>

      <CtaSection />
    </>
  );
}
